...
- When a comment or code cites a documentation link, open and verify it before drawing conclusions; if you can't confirm it, downgrade the claim from assertion to suspicion and say so.
...
- Prefer WebFetch / WebSearch. Fallback to `firecrawl` skill if cannot get the links' real content.
